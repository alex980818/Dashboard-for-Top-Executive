...
// By Today
    if($dateOption=="today"){
        ...
        $i=0;
                ...
                $sql2 = "SELECT PRODUCT, DATE, SUM(COST) AS TOTALCOST FROM PRODUCTS WHERE PRODUCT = 'A' AND 
                ...
                $sql2 = "SELECT PRODUCT, DATE, SUM(COST) AS TOTALCOST FROM PRODUCTS WHERE PRODUCT = 'B' AND 
                DATE BETWEEN '$lastday' AND '$nowDay' GROUP BY MONTH(DATE) ORDER BY DATE";
                $result2 = mysqli_query($conn, $sql2);
                      while($row = mysqli_fetch_array($result2)){
                            $nowMonth = date("Y/m", strtotime($row["DATE"]));
                ...
                $sql2 = "SELECT PRODUCT, DATE, SUM(COST) AS TOTALCOST FROM PRODUCTS WHERE PRODUCT = 'C' AND 
                DATE BETWEEN '$lastday' AND '$nowDay' GROUP BY MONTH(DATE) ORDER BY DATE";
                $result2 = mysqli_query($conn, $sql2);
                      while($row = mysqli_fetch_array($result2)){
                            $nowMonth = date("Y/m", strtotime($row["DATE"]));
                ...
                $sql2 = "SELECT PRODUCT, DATE, SUM(COST) AS TOTALCOST FROM PRODUCTS WHERE PRODUCT = 'D' AND 
                DATE BETWEEN '$lastday' AND '$nowDay' GROUP BY MONTH(DATE) ORDER BY DATE";
                $result2 = mysqli_query($conn, $sql2);
                      while($row = mysqli_fetch_array($result2)){
                            $nowMonth = date("Y/m", strtotime($row["DATE"]));
...
function drawChart () {
    var data = google.visualization.arrayToDataTable([
       ...
       for($j=0; $j<$i; $j++){ ?>                                          
                ['<?echo $product[$j] ?>', '<?echo $totalDate[$j] ?>', <?echo $cost[$j]  ?>],
      <?php }?>
    ...
    function draw (category) {
        if (topLevel) {
            // rename the title
            options.title = 'Cost of Product (RM)';
