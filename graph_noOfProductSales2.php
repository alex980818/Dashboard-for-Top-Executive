...
// By Today
    if($dateOption=="today"){
        ...
        $i=0;
                ...
                $sql2 = "SELECT PRODUCT, DATE, SUM(PRODSALES) AS TOTALPRODSALES FROM PRODUCTS WHERE PRODUCT = 'A' AND 
                ...
                $result2 = mysqli_query($conn, $sql2);
                      while($row = mysqli_fetch_array($result2)){
                            ...
                            $prodSales[$i] = $row["TOTALPRODSALES"];
                ...
                $sql2 = "SELECT PRODUCT, DATE, SUM(PRODSALES) AS TOTALPRODSALES FROM PRODUCTS WHERE PRODUCT = 'B' AND 
                DATE BETWEEN '$lastday' AND '$nowDay' GROUP BY MONTH(DATE) ORDER BY DATE";
                $result2 = mysqli_query($conn, $sql2);
                      while($row = mysqli_fetch_array($result2)){
                            $nowMonth = date("Y/m", strtotime($row["DATE"]));
                ...
                $sql2 = "SELECT PRODUCT, DATE, SUM(PRODSALES) AS TOTALPRODSALES FROM PRODUCTS WHERE PRODUCT = 'C' AND 
                DATE BETWEEN '$lastday' AND '$nowDay' GROUP BY MONTH(DATE) ORDER BY DATE";
                $result2 = mysqli_query($conn, $sql2);
                      while($row = mysqli_fetch_array($result2)){
                            $nowMonth = date("Y/m", strtotime($row["DATE"]));
                ...
                $sql2 = "SELECT PRODUCT, DATE, SUM(PRODSALES) AS TOTALPRODSALES FROM PRODUCTS WHERE PRODUCT = 'D' AND 
                DATE BETWEEN '$lastday' AND '$nowDay' GROUP BY MONTH(DATE) ORDER BY DATE";
                $result2 = mysqli_query($conn, $sql2);
                      while($row = mysqli_fetch_array($result2)){
                            $nowMonth = date("Y/m", strtotime($row["DATE"]));
...
<!-------------noOfProductSales for last7day, last30day, last12month------------------->
...
function drawChart () {
    var data = google.visualization.arrayToDataTable([
       ...
       for($j=0; $j<$i; $j++){ ?>                                          
                ['<?echo $product[$j] ?>', '<?echo $totalDate[$j] ?>', <?echo $prodSales[$j] ?>],
      <?php }?>
...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    ...
    function drawChart2() {
      var data = google.visualization.arrayToDataTable([
        ...
        ["Product D", <?php echo $totalC+$totalA ?>],
      ...
      var options = {
        title: "Number of Product Sales (Unit)",
        // 'width':'100%',
        // height: 350,
        // bar: {groupWidth: "95%"},
...
<body style="overflow: hidden;">
      ...
      <?php
        if($dateOption=="last7day" || $dateOption=="last30day" ||$dateOption=="last12month"){
            ?>
            <div id="chart" style="width: 98%; height: 98%;">
            <?php
        }else{?>
